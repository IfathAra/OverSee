...
while True:
    ...
    #for (x, y, w, h) in rects:
       # rect = dlib.rectangle(int(x), int(y), int(x + w),int(y + h))
        result1 = DeepFace.analyze(frame,actions = ['emotion'])
        text=result1['dominant_emotion']
        if(text == 'sad' or text=='surprise'):
            text = 'neutral'
        if(text=='angry' or text =='disgust' or text=='fear' or 'neutral'):
            if(text=='fear'):
                text='disgust'
